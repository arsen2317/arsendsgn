...
- Next.js 14 App Router. Single root layout (`app/layout.js`) + homepage (`app/page.js`) composed of section components from `components/` (Hero, Intro, Skills, Cases, Footer, etc.), plus case study pages under `app/<name>/page.js` (e.g. `app/sber`, `app/vibes`, `app/tj`), routed at `/<name>` (no `/cases` prefix).
